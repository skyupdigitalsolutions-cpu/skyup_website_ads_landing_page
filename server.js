// SkyUp ads landing page — Railway backend (Express + MongoDB Atlas)
// Serves the built site from /dist AND the lead API from the same origin (no CORS needed).
...
app.post('/api/lead', async (req, res) => {
  ...
  // fire-and-forget integrations (never block the visitor)
  notifyTelegram(lead)
  forwardToCrm(lead)
  res.json({ ok: true })
})
...
// --- Static site (built by `npm run build`) ---
const distDir = path.join(__dirname, 'dist')
app.use(express.static(distDir))
// single-page fallback for any non-API route
app.get('*', (req, res) => res.sendFile(path.join(distDir, 'index.html')))

app.listen(PORT, () => console.log(`SkyUp landing running on :${PORT}`))
